...
class RollbackServiceTest {
	...
	@BeforeEach
	void init() {
		rollbackService = new RollbackService(elasticSearchRepository, rabbitMqService, specimenRepository,
				mediaRepository, fdoRecordService, handleComponent);
	}
	...
	void testRollbackNewSpecimensCase1() {
		...
		// When
		rollbackService.rollbackNewSpecimens(Set.of(givenDigitalSpecimenRecord()), false, false);
	...
	void testRollbackNewSpecimensCase3ElasticFails() throws Exception {
		// Given
		doThrow(ElasticsearchException.class).when(elasticSearchRepository).rollbackObject(HANDLE, true);
	...
	void testRollbackNewMediaCase3ElasticFails() throws Exception {
		// Given
		doThrow(ElasticsearchException.class).when(elasticSearchRepository).rollbackObject(MEDIA_PID, false);
	...
	void testRollbackUpdatedMediaCase1() {
		...
		// When
		rollbackService.rollbackUpdatedMedias(Set.of(givenUpdatedDigitalMediaRecord()), false, false);
	...
	void testRollbackUpdatedMediaCase2() {
		...
		// When
		rollbackService.rollbackUpdatedMedias(Set.of(givenUpdatedDigitalMediaRecord()), false, true);
	...
	void testRollbackUpdatedMediaCase3() throws Exception {
		...
		// When
		rollbackService.rollbackUpdatedMedias(Set.of(givenUpdatedDigitalMediaRecord()), true, true);
	...
	void testHandlePartiallyFailedInsertMedia() {
		...
		// When
		var result = rollbackService.handlePartiallyFailedElasticInsertMedia(records, bulkResponse);
	...
	void testHandlePartiallyFailedElasticUpdateSpecimen() throws Exception {
		...
		var failedRecord = new UpdatedDigitalSpecimenRecord(
				givenUnequalDigitalSpecimenRecord(SECOND_HANDLE, ANOTHER_SPECIMEN_NAME, ORGANISATION_ID, false,
						PHYSICAL_SPECIMEN_ID_ALT, true),
				Set.of(MAS), givenDigitalSpecimenRecord(SECOND_HANDLE, PHYSICAL_SPECIMEN_ID_ALT, false),
