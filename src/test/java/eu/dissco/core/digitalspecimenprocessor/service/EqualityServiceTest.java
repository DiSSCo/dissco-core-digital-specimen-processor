...
class EqualityServiceTest {
	...
	void testEqualSpecimens(DigitalSpecimenRecord currentDigitalSpecimen, DigitalSpecimenWrapper digitalSpecimen,
			MediaRelationshipProcessResult mediaProcessResult) {

		// When
		var result = equalityService.specimensAreEqual(currentDigitalSpecimen, digitalSpecimen, mediaProcessResult);
	...
	@ParameterizedTest
	@MethodSource("provideEqualSpecimens")
	void testParser(DigitalSpecimenRecord currentDigitalSpecimen, DigitalSpecimenWrapper digitalSpecimen,
			MediaRelationshipProcessResult mediaProcessResult) {
	...
	void testEqualMedia(DigitalMediaRecord currentDigitalMedia, DigitalMediaWrapper digitalMediaWrapper) {
		// Given

		// When
		var result = equalityService.mediaAreEqual(currentDigitalMedia, digitalMediaWrapper, Set.of());
	...
	void testUnequalMedia() {
		// Given
		var wrapper = new DigitalMediaWrapper(TYPE_MEDIA, givenUnequalDigitalMedia(MEDIA_URL, true),
	...
	void testUnequalMediaNewSpecimen() {
		...
		// When
		var result = equalityService.mediaAreEqual(givenDigitalMediaRecord(), givenDigitalMediaWrapper(),
				Set.of(HANDLE));
	...
	void testTombstonedMediaRelationship() {
		...
		var currentDigitalSpecimen = givenDigitalSpecimenRecord();
		var digitalSpecimen = givenDigitalSpecimenWrapper();
		var mediaProcessResult = new MediaRelationshipProcessResult(List.of(new EntityRelationship()), List.of(),
				List.of());
	...
	void testSetEntityRelationshipDate() {
		...
		var digitalSpecimenEvent = new DigitalSpecimenEvent(Set.of(MAS), digitalSpecimenWrapper,
				List.of(givenDigitalMediaEvent()), false, true);
		var mediaEr = givenEntityRelationship(MEDIA_PID, HAS_MEDIA.getRelationshipName());

		var attributes = givenDigitalSpecimenWrapper(true).attributes()
			.withDctermsCreated(null)
			.withOdsIsKnownToContainMedia(true)
			.withDctermsModified(UPDATED_STR)
			.withOdsHasEntityRelationships(currentDigitalSpecimen.attributes().getOdsHasEntityRelationships());
		var expected = new DigitalSpecimenEvent(Set.of(MAS),
				new DigitalSpecimenWrapper(PHYSICAL_SPECIMEN_ID, TYPE_PID, attributes, ORIGINAL_DATA),
				List.of(givenDigitalMediaEvent()), false, true);

		// When
		var result = equalityService.setExistingEventDatesSpecimen(currentDigitalSpecimen, digitalSpecimenEvent,
				new MediaRelationshipProcessResult(List.of(), List.of(), List.of(mediaEr)));
	...
	void testSetEntityRelationshipDateMedia() {
		...
		var expectedWrapper = new DigitalMediaWrapper(TYPE_MEDIA,
				givenDigitalMedia(MEDIA_URL, false).withOdsHasEntityRelationships(List.of(entityRelationship)),
				MAPPER.createObjectNode());
		var expected = new DigitalMediaEvent(Set.of(MEDIA_MAS), expectedWrapper, false, true);
		var event = new DigitalMediaEvent(Set.of(MEDIA_MAS), changeTimestamps(expectedWrapper), false, true);
		var currentRecord = new DigitalMediaRecord(MEDIA_PID, MEDIA_URL, VERSION, CREATED, Set.of(MEDIA_MAS),
				givenDigitalMedia(MEDIA_URL, false).withOdsHasEntityRelationships(List.of(entityRelationship)),
	...
	private static Stream<Arguments> provideEqualSpecimens() {
		return Stream.of(
				Arguments.of(givenDigitalSpecimenRecord(), changeTimestamps(givenDigitalSpecimenWrapper()),
						givenEmptyMediaProcessResult()),
				Arguments.of(givenDigitalSpecimenRecordWithMediaEr(HANDLE, PHYSICAL_SPECIMEN_ID, true),
						changeTimestamps(givenDigitalSpecimenWrapper(true, true)), givenEmptyMediaProcessResult()),
				Arguments.of(givenDigitalSpecimenRecord(),
						changeTimestamps(givenDigitalSpecimenWrapperNoOriginalData()), givenEmptyMediaProcessResult()));
	}

	private static Stream<Arguments> provideEqualMedia() {
		return Stream.of(Arguments.of(givenDigitalMediaRecord(), givenDigitalMediaWrapper()),
				Arguments.of(givenDigitalMediaRecord(), changeTimestamps(givenDigitalMediaWrapper())));
	}

	private static DigitalSpecimenWrapper changeTimestamps(DigitalSpecimenWrapper digitalSpecimenWrapper) {
		var attributes = digitalSpecimenWrapper.attributes();
		attributes.withDctermsCreated(UPDATED).withDctermsModified(UPDATED_STR);
		if (attributes.getOdsHasEntityRelationships() != null) {
			attributes.getOdsHasEntityRelationships().forEach(er -> er.setDwcRelationshipEstablishedDate(UPDATED));
		}
	...
	private static DigitalMediaWrapper changeTimestamps(DigitalMediaWrapper digitalMediaWrapper) {
		var attributes = digitalMediaWrapper.attributes();
		attributes.withDctermsCreated(UPDATED);
		if (attributes.getOdsHasEntityRelationships() != null) {
			attributes.getOdsHasEntityRelationships().forEach(er -> er.setDwcRelationshipEstablishedDate(UPDATED));
		}
		return new DigitalMediaWrapper(digitalMediaWrapper.type(), attributes,
				digitalMediaWrapper.originalAttributes());
