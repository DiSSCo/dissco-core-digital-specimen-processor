...
class AnnotationPublisherServiceTest {
	...
	public static Stream<Arguments> provideUpdateAnnotations() {
		var classObject = MAPPER.readValue("""
				...
				""", JsonNode.class);
		return Stream.of(Arguments.of(MAPPER.readTree("""
				  [{
				    "op": "replace",
				...
				Arguments.of(classObject,
						List.of(givenAcceptedAnnotation(OaMotivation.ODS_ADDING,
								new OaHasSelector().withAdditionalProperty("@type", "ods:ClassSelector")
									.withAdditionalProperty("ods:class", "$['ods:hasEntityRelationship']"),
								new AnnotationBody()
									.withOaValue(List.of(MAPPER.writeValueAsString(classObject.get(0).get("value"))))
									.withType("oa:TextualBody")
									.withDctermsReferences(SOURCE_SYSTEM_ID)))),
				...
				Arguments.of(MAPPER.readTree("""
						...
						List.of(givenAcceptedAnnotation(OaMotivation.ODS_ADDING,
								new OaHasSelector().withAdditionalProperty("@type", "ods:TermSelector")
									.withAdditionalProperty("ods:term", "$['ods:sourceSystemName']"),
					...
					.of(MAPPER.readTree("""
							...
							List.of(givenAcceptedAnnotation(OaMotivation.ODS_ADDING,
									...
									new AnnotationBody().withOaValue(List.of(SOURCE_SYSTEM_ID))
										.withType("oa:TextualBody")
										.withDctermsReferences(SOURCE_SYSTEM_ID)),
									givenAcceptedAnnotation(OaMotivation.ODS_DELETING,
											new OaHasSelector().withAdditionalProperty("@type", "ods:TermSelector")
												.withAdditionalProperty("ods:term", "$['ods:sourceSystemID']"),
											null))));
	}

	private static AnnotationProcessingRequest givenAcceptedAnnotation(OaMotivation motivation, OaHasSelector selector,
			AnnotationBody body) {
		var annotation = new AnnotationProcessingRequest().withOaMotivation(motivation)
			.withOaHasBody(body)
			.withOaHasTarget(new AnnotationTarget().withOdsFdoType(TYPE_PID)
				.withType(TYPE)
		...
		if (motivation == OaMotivation.OA_EDITING) {
			annotation.withOaMotivatedBy("Received update information from Source System with id: " + SOURCE_SYSTEM_ID);
		}
		if (motivation == OaMotivation.ODS_ADDING) {
			annotation.withOaMotivatedBy("Received new information from Source System with id: " + SOURCE_SYSTEM_ID);
		}
		if (motivation == OaMotivation.ODS_DELETING) {
			annotation.withOaMotivatedBy("Received delete information from Source System with id: " + SOURCE_SYSTEM_ID);
		}
		return annotation;
	}
	...
	void testPublishAnnotationUpdatedSpecimen(JsonNode jsonPatch,
		...
		// When
		service.publishAnnotationUpdatedSpecimen(Set.of(new UpdatedDigitalSpecimenRecord(givenDigitalSpecimenRecord(),
				Set.of(), null, jsonPatch, List.of(), givenEmptyMediaProcessResult(), true)));

		// Then
		for (var expectedAnnotation : expectedAnnotations) {
			then(rabbitMQService).should().publishAcceptedAnnotation(givenAutoAcceptedAnnotation(expectedAnnotation));
		}
	}
	...
	void testPublishAnnotationUpdatedSpecimenMultiple() {
		...
		// When
		service.publishAnnotationUpdatedSpecimen(Set.of(new UpdatedDigitalSpecimenRecord(givenDigitalSpecimenRecord(),
				Set.of(), null, givenLargeJsonPatch(), List.of(), givenEmptyMediaProcessResult(), true)));

		// Then
		then(rabbitMQService).should(times(29)).publishAcceptedAnnotation(givenAutoAcceptedAnnotation(any()));
	}
