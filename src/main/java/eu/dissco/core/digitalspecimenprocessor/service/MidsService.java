...
public class MidsService {
	...
	private static final List<String> UNACCEPTED_VALUES = List.of("null", "unknown", "unknown:undigitized");

	private static final List<OdsTopicDiscipline> BIO_DISCIPLINES = List.of(OdsTopicDiscipline.BOTANY,
			OdsTopicDiscipline.MICROBIOLOGY, OdsTopicDiscipline.ZOOLOGY, OdsTopicDiscipline.OTHER_BIODIVERSITY);

	private static final List<OdsTopicDiscipline> PALEO_GEO_DISCIPLINES = List.of(OdsTopicDiscipline.PALAEONTOLOGY,
			OdsTopicDiscipline.ASTROGEOLOGY, OdsTopicDiscipline.GEOLOGY, OdsTopicDiscipline.ECOLOGY,
			OdsTopicDiscipline.OTHER_GEODIVERSITY);
	...
	private boolean compliesToMidsOne(DigitalSpecimen attributes) {
		return isValid(attributes.getDctermsLicense()) && isValid(attributes.getDctermsModified())
				&& isValid(attributes.getDwcPreparations()) && attributes.getOdsTopicDiscipline() != null
	...
	private boolean compliesToMidsThree(DigitalSpecimen attributes) {
		if (PALEO_GEO_DISCIPLINES.contains(attributes.getOdsTopicDiscipline())) {
			...
			return checkMidsThreeGeneric(attributes) && hasAgentWithRole(COLLECTOR,
					attributes.getOdsHasEvents().stream().map(Event::getOdsHasAgents).flatMap(List::stream).toList(),
					true);
		}
	}
	...
	private boolean geographicalValuesComply(DigitalSpecimen attributes) {
		if (attributes.getOdsHasEvents() != null
			...
			if (isValid(location.getDwcLocationID())) {
				...
				return (geoReference.getDwcDecimalLatitude() != null && geoReference.getDwcDecimalLongitude() != null
						...
						|| (isValid(geoReference.getDwcFootprintWKT()) && isValid(geoReference.getDwcFootprintSRS()));
			}
		}
		return false;
	}
	...
	private boolean compliesToMidsTwoBio(DigitalSpecimen attributes) {
		return Boolean.TRUE.equals(attributes.getOdsIsKnownToContainMedia()) && qualitativeLocationIsValid(attributes)
				&& quantitativeLocationIsValid(attributes)
				&& (eventIsPresent(attributes) && hasValue(attributes.getOdsHasEvents().getFirst().getDwcEventDate(),
						attributes.getOdsHasEvents().getFirst().getDwcVerbatimEventDate(),
						convertInteger(attributes.getOdsHasEvents().getFirst().getDwcYear())))
				&& (eventIsPresent(attributes) && hasValue(attributes.getOdsHasEvents().getFirst().getDwcFieldNumber())
						&& hasAgentWithRole(COLLECTOR,
								attributes.getOdsHasEvents()
									.stream()
									.map(Event::getOdsHasAgents)
									.flatMap(List::stream)
									.toList(),
								false));
	}
	...
	private boolean compliesToMidsTwoPaleoBio(DigitalSpecimen attributes) {
		return attributes.getOdsIsMarkedAsType() != null && stratigraphyIsValid(attributes)
				&& qualitativeLocationIsValid(attributes) && quantitativeLocationIsValid(attributes);
	}

	private boolean quantitativeLocationIsValid(DigitalSpecimen digitalSpecimen) {
		if (locationIsPresent(digitalSpecimen)
				&& digitalSpecimen.getOdsHasEvents().getFirst().getOdsHasLocation().getOdsHasGeoreference() != null) {
			...
			return hasValue(location.getDwcLocationID(), geoReference.getDwcFootprintWKT())
					|| (geoReference.getDwcDecimalLatitude() != null && geoReference.getDwcDecimalLongitude() != null);
		}
		return false;
	}
	...
	private boolean qualitativeLocationIsValid(DigitalSpecimen digitalSpecimen) {
		if (locationIsPresent(digitalSpecimen)) {
			...
			var location = digitalSpecimen.getOdsHasEvents().getFirst().getOdsHasLocation();
			isValidValue = hasValue(location.getDwcContinent(), location.getDwcCountry(), location.getDwcCountryCode(),
					location.getDwcCounty(), location.getDwcIsland(), location.getDwcIslandGroup(),
					location.getDwcLocality(), location.getDwcVerbatimLocality(), location.getDwcMunicipality(),
					location.getDwcStateProvince(), location.getDwcWaterBody(), location.getDwcHigherGeography());
			if (!isValidValue && (location.getOdsHasGeoreference() != null)) {
				...
				if (!isValidValue) {
					isValidValue = (isValid(location.getOdsHasGeoreference().getDwcVerbatimLatitude())
							&& isValid(location.getOdsHasGeoreference().getDwcVerbatimLongitude()));
				}
			}
			return isValidValue;
		}
	...
	private boolean stratigraphyIsValid(DigitalSpecimen digitalSpecimen) {
		if (locationIsPresent(digitalSpecimen) && digitalSpecimen.getOdsHasEvents()
			.getFirst()
			.getOdsHasLocation()
			.getOdsHasGeologicalContext() != null) {
			var geologicalContext = digitalSpecimen.getOdsHasEvents()
				.getFirst()
				.getOdsHasLocation()
				.getOdsHasGeologicalContext();
			return hasValue(geologicalContext.getDwcBed(), geologicalContext.getDwcMember(),
					geologicalContext.getDwcFormation(), geologicalContext.getDwcGroup(),
