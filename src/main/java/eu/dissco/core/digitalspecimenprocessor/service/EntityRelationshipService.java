...
public class EntityRelationshipService {
	...
	public MediaRelationshipProcessResult processMediaRelationshipsForSpecimen(
			Map<String, DigitalSpecimenRecord> currentSpecimens, DigitalSpecimenEvent digitalSpecimenEvent,
			Map<String, DigitalMediaRecord> currentMedia) {
		// Media Uri -> DOI
		var mediaIdMap = getExistingMediaIdMap(currentMedia);
		var currentSpecimen = currentSpecimens.get(digitalSpecimenEvent.digitalSpecimenWrapper().physicalSpecimenID());
		...
		var tombstonedMediaErs = getTombstonedMediaRelationships(digitalSpecimenEvent, currentMediaErs, mediaIdMap);
		var unchangedMediaErs = getUnchangedMediaEntityRelationships(tombstonedMediaErs, currentMediaErs);
		return new MediaRelationshipProcessResult(tombstonedMediaErs, newMediaErs, unchangedMediaErs);
	}

	public Set<String> findNewSpecimenRelationshipsForMedia(DigitalMediaRecord digitalMediaRecord,
			PidProcessResult pidProcessResult) {
		if (digitalMediaRecord.attributes().getOdsHasEntityRelationships() == null) {
			return Set.of();
		}
		var existingLinkedPids = digitalMediaRecord.attributes()
		...
		return pidProcessResult.doisOfRelatedObjects()
			.stream()
			.filter(Objects::nonNull)
			.filter(relatedDoi -> !existingLinkedPids.contains(relatedDoi))
			.collect(Collectors.toSet());
	}

	/*
	 * In our (incoming) mediaEvents, we are missing mediaID but we have the url In the
	 * existing entity relationships in the specimen, we are missing the media url but we
	 * have the id Returns Map<Media DOI, URI>
	 */
	private Map<String, String> getExistingMediaIdMap(Map<String, DigitalMediaRecord> currentMedia) {
		return currentMedia.entrySet()
			.stream()
			.collect(Collectors.toMap(entry -> entry.getValue().id(), Entry::getKey));
	}

	private List<DigitalMediaEvent> getNewLinkedMedia(DigitalSpecimenEvent digitalSpecimenEvent,
			Map<String, String> mediaIdMap, List<EntityRelationship> currentMediaErs) {
		var currentMediaUris = currentMediaErs.stream().map(er -> {
		...
		return digitalSpecimenEvent.digitalMediaEvents()
			.stream()
			.filter(event -> !currentMediaUris.contains(event.digitalMediaWrapper().attributes().getAcAccessURI()))
			.toList();
	}

	// Identify which media ERs in the previous version are no longer relevant
	private List<EntityRelationship> getTombstonedMediaRelationships(DigitalSpecimenEvent digitalSpecimenEvent,
			List<EntityRelationship> currentMediaErs, Map<String, String> mediaIdMap) {
		// Get media URIs from this batch
		var incomingMediaUris = digitalSpecimenEvent.digitalMediaEvents()
			.stream()
			.map(event -> event.digitalMediaWrapper().attributes().getAcAccessURI())
			.toList();
		// If a media ER is NOT associated with a URI that is in this current batch, that
		// means the relationship no longer exists
		return currentMediaErs.stream().filter(er -> {
			...
			var mediaUri = mediaIdMap.get(er.getDwcRelatedResourceID().replace(DOI_PROXY, ""));
